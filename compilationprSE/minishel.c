...
/*
   ...
   permet ŕ child_signal de notifier ŕ execute que la commande au premier
   plan vient de se terminer

   la variable étant partagée entre le programme principal (lance_commande) et
   un handler de signal asynchrone (child_signal), elle dont ętre marquée volatile
*/
////===========================================================================================
volatile int fg_pid = -1;
////===========================================================================================
void child_signal(int signal) {
    /* un signal peut correspondre ŕ plusieurs fils finis, donc on boucle */
    while (1) {
        printf(" wait ");
        int status;
        ...
        /*  signale ŕ execute que fg_pid s'est terminé */
...
void execute(char* elem[]) {
    ...
    if (!elem[0]) exit;
    if (!en_fond) {
        /* bloque child_signal jusqu'ŕ ce que le pčre ait placé le pid du
           fils dans fg_pid
    ...
    if ((strncmp(elem[0],"help", 4))==0) { //strncmp returns 0 if equal
        ...
        printf("Ces commandes de shell sont définies de maničre interne.Tapez Ť help ť pour voir cette liste.\n");
        printf("Utilisez Ť man ť ou Ť info ť pour en savoir plus sur les commandes qui\n");
    ...
    if (!Exit) {
        ...
        if (pid==0) {
            ...
            if (use_glob) {
                ...
                execvp(elem[0], /* programme ŕ exécuter */
                       elem    /* argv du programme ŕ exécuter */
                      );
            ...
            if (!en_fond) {
                ...
                /* attente bloquante jusqu'ŕ ce que child_signal signale que fg_pid
                   s'est terminé */
                while (fg_pid!=-1) {
                    //		pause();
                    fg_pid = pid;
